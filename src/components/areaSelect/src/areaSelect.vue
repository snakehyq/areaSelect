...
const cictySelectRef = ref<InstanceType<typeof Select> | null>(null);
const countrySelectRef = ref<InstanceType<typeof Select> | null>(null);
// 策略性数据
const provinces = formatData(data);
let modelValueMap: number[] = [];
...
function echoSelect() {
  ...
  function echoProvinceSelect() {
    ...
    console.log(
      "provinces[provinceId as number]",
      provinces[provinceId as number]
    );
...
const handleProvincesSelected = (value: string) => {
  ...
  if (state.selectedInfo.city) {
    state.selectedInfo.city = null;
    state.selectedInfo.country = null;
    cictySelectRef.value.resetSelected();
    countrySelectRef.value.resetSelected();
  }
  modelValueMap = [Number(code)];
  emits("handleSelected", toRaw(state.selectedInfo));
...
const handleCictiesSelected = (value: string) => {
  ...
  if(state.selectedInfo.country) {
    state.selectedInfo.country = null;
    countrySelectRef.value.resetSelected();
  }
  modelValueMap = [modelValueMap[0], Number(code)];
  emits("handleSelected", toRaw(state.selectedInfo));
  emits("update:modelValue", modelValueMap);
};
const handleCountiesSelected = (value: string) => {
  ...
  modelValueMap = [modelValueMap[0], modelValueMap[1], Number(code)];
  emits("handleSelected", toRaw(state.selectedInfo));
  emits("update:modelValue", modelValueMap);
};
</script>
